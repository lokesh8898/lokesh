...
if len(sys.argv) > 1:
    ...
    # Usage: python processToCSV.py [NFO_FOLDER] [BFO_FOLDER] [OUTPUT_FOLDER]
    if len(sys.argv) >= 2:
    ...
    if len(sys.argv) >= 4:
        config.OUTPUT_CSV_DIR = sys.argv[3]
...
for dirr in [config.OUTPUT_CSV_DIR]:
...
logging.info("Started processing GDFL data to CSV.")
...
logging.info(f"Output will be saved to: {config.OUTPUT_CSV_DIR}")

for instu in instutype:
    for indice in indexname:
        logging.info(f"Processing {indice} {instu}...")
        subprocess.run(['python', "prepareTickByTickData_CSV.py", indice, instu])

logging.info(f"Processing complete. CSV files saved in '{config.OUTPUT_CSV_DIR}' folder.")
