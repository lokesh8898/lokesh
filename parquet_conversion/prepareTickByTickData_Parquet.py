...
def getDateFolder(exchange: str) -> dict:
    ...
    # Check if the GDFL folder itself is a dated folder (e.g., GFDLNFO_TICK_03012025  or BFO_TICK_03012025)
    folderName = os.path.basename(gdfl_folder)
    ...
    # Final fallback: use DEFAULT_TRADING_DATE from config if no date found
    if not toreturn and hasattr(config, 'DEFAULT_TRADING_DATE'):
        logging.info(f"No date found in folder structure, using DEFAULT_TRADING_DATE: {config.DEFAULT_TRADING_DATE}")
        toreturn[config.DEFAULT_TRADING_DATE] = {gdfl_folder}
    
    return toreturn

def getFinalDatasetToPush(dataFilePath: str, optionInfo: dict, isFutureDataFile: bool) -> pd.DataFrame:
    ...
    try:
        ...
        logging.info(f"Successfully processed: {dataFilePath}")
        return optDataset
...
if __name__ == "__main__":
    ...
    os.makedirs(config.OUTPUT_PARQUET_DIR, exist_ok=True)

    if len(sys.argv) != 3:
        logging.info("Command line parameters are not passed properly. Usage: python prepareTickByTickData_Parquet.py <INDEX> <instrument_type>")
        exit()
        ...
        for folderpath in DATE_FOLDERS[tradingdatekey]:
            ...
            if uPara['instrument_type'] == "future":
                ...
                if len(futurefilepath) >= 1:
                    outputFilePath = os.path.join(config.OUTPUT_PARQUET_DIR, f"{uPara['index'].lower()}_future_{tradingdatekey}.parquet")
                    
                    # Collect all dataframes first
                    dataframes_list = []
                    for future_file in futurefilepath:
                        df = getFinalDatasetToPush(dataFilePath=os.path.join(folderpath, future_file), optionInfo={
                    ...
                    if dataframes_list:
                        ...
                        combined_df.to_parquet(outputFilePath, index=False, engine='pyarrow', compression='snappy')
                        logging.info(f"Saved {len(dataframes_list)} future files to {outputFilePath} ({len(combined_df)} rows)")
                else:
                ...
                outputFilePath = os.path.join(config.OUTPUT_PARQUET_DIR, f"{uPara['index'].lower()}_{uPara['instrument_type']}_{tradingdatekey}.parquet")

                # Collect all dataframes first (much faster than appending to parquet each time)
                dataframes_list = []
                processed_count = 0
                    ...
                    try:
                        ...
                        if df is not None:
                            ...
                            if processed_count % 100 == 0:
                                logging.info(f"Processed {processed_count}/{len(symbolFiles)} files...")
                    except Exception as e:
                        logging.error(f"Error processing {symbolFile}: {str(e)}\n{traceback.format_exc()}")
                
                # Write all data at once - MUCH faster than incremental appends
                if dataframes_list:
                    logging.info(f"Combining {len(dataframes_list)} dataframes...")
                    combined_df = pd.concat(dataframes_list, ignore_index=True)
                    logging.info(f"Writing to {outputFilePath} ({len(combined_df)} rows)...")
                    combined_df.to_parquet(outputFilePath, index=False, engine='pyarrow', compression='snappy')
                    logging.info(f"Successfully saved {processed_count} option files to {outputFilePath}")
