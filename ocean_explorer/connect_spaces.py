...
class DigitalOceanSpaces:
    ...
    def test_connection(self):
        ...
        try:
            ...
            print("[OK] Successfully connected to DigitalOcean Spaces!")
            print(f"[OK] Endpoint: {self.endpoint_url}")
            print(f"[OK] Region: {self.region}")
            print(f"\nAvailable buckets:")
            ...
            print(f"[ERROR] Error connecting to Spaces: {e}")
            return False
    ...
    def explore_directory(self, bucket_name, prefix='', delimiter='/'):
        ...
        try:
            ...
            print(f"[ERROR] Error exploring directory: {e}")
            return None
    ...
    def get_object_metadata(self, bucket_name, object_key):
        ...
        try:
            ...
            print(f"[ERROR] Error getting object metadata: {e}")
            return None
    ...
    def get_bucket_stats(self, bucket_name, prefix=''):
        ...
        try:
            ...
            print(f"[ERROR] Error getting bucket stats: {e}")
            return None
    ...
    def upload_file(self, local_file_path, bucket_name, object_name, silent=False):
        """
        ...
        try:
            ...
            if file_size == 0:
                warning_msg = f"File is empty: {local_file_path}"
                if not silent:
                    print(f"[WARNING] {warning_msg}")
                # Still try to upload empty file
            
            # Upload file to Spaces
            self.s3_client.upload_file(local_file_path, bucket_name, object_name)
            
            if not silent:
                print(f"[OK] Successfully uploaded {local_file_path} ({file_size:,} bytes) to {bucket_name}/{object_name}")
            return True
            ...
            full_error = f"ClientError [{error_code}]: {error_msg}"
            if not silent:
                print(f"[ERROR] Error uploading file: {full_error}")
            # Store error for caller to access
            self._last_upload_error = full_error
            raise  # Re-raise so caller can see the actual error
        except Exception as e:
            error_msg = f"Unexpected error: {type(e).__name__}: {str(e)}"
            if not silent:
                print(f"[ERROR] Unexpected error uploading file: {error_msg}")
            # Store error for caller
            self._last_upload_error = error_msg
            raise  # Re-raise so caller can see the actual error
    ...
    def download_file(self, bucket_name, object_name, local_file_path):
        ...
        try:
            ...
            print(f"[OK] Successfully downloaded {bucket_name}/{object_name} to {local_file_path}")
            return True
        except ClientError as e:
    ...
    def delete_file(self, bucket_name, object_name):
        ...
        try:
            ...
            print(f"[OK] Successfully deleted {bucket_name}/{object_name}")
            return True
        except ClientError as e:
